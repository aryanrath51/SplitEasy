...
interface Expense {
  ...
  splitAmong: number[];
  splitAmounts: { [key: number]: number };
...
// --- FIX: Wrapper outside component prevents input focus loss while typing ---
const MainWrapper = ({ children, isDarkMode }: { children: React.ReactNode, isDarkMode: boolean }) => (
  <div className={isDarkMode ? "dark" : ""}>
    <div className="min-h-screen transition-all duration-500 bg-gradient-to-br from-slate-50 via-white to-blue-100 dark:from-slate-950 dark:via-slate-900 dark:to-blue-950 text-slate-900 dark:text-slate-100">
      {children}
    </div>
  </div>
);
...
export default function HomePage() {
  ...
  const [newExpense, setNewExpense] = useState({
    ...
    splitAmounts: {} as { [key: number]: string },
  });
  ...
  // --- RESTORED FUNCTIONS (Fixes image_4c7e45 & image_4babe9) ---
  const handleAuth = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setUser(data.user);
    } catch (err: any) { setAuthError("Invalid credentials"); }
  };
  ...
  const settlements = useMemo(() => {
    ...
    selectedGroup.expenses.forEach(e => {
      ...
      e.splitAmong.forEach(mId => {
        balances[mId] -= e.split_type === 'custom' ? (e.splitAmounts?.[mId] || 0) : (e.amount / e.splitAmong.length);
      });
    });
  ...
  // --- RESTORED TOTAL (Fixes image_4c7a27) ---
  const totalExpenses = useMemo(() => {
    return selectedGroup?.expenses.reduce((s, e) => s + e.amount, 0) || 0;
  }, [selectedGroup]);
  ...
  const addExpense = async () => {
    ...
    await fetch(`/api/groups/${selectedGroup.id}/expenses`, {
      ...
      body: JSON.stringify({ ...newExpense, amount: parseFloat(newExpense.amount), splitAmounts: splitAmountsData, date: new Date().toISOString().split("T")[0] }),
    });
  ...
  if (!user) return (
    <MainWrapper isDarkMode={isDarkMode}>
      <div className="flex items-center justify-center min-h-screen p-6">
        <Card className="p-10 bg-white/90 dark:bg-slate-900/90 border-slate-200 dark:border-slate-800 max-w-lg w-full text-center shadow-2xl rounded-3xl">
          ...
          <form onSubmit={handleAuth} className="space-y-6 text-left">
            ...
            <Button className="w-full h-12 text-lg font-bold bg-blue-600 hover:bg-blue-700 rounded-xl">Continue</Button>
  ...
  if (!selectedGroupId) return (
    <MainWrapper isDarkMode={isDarkMode}>
      <header className="bg-white/80 dark:bg-slate-900/80 border-b dark:border-slate-800 sticky top-0 py-5 z-10">
        <div className="max-w-5xl mx-auto px-6 flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            <button onClick={() => setIsDarkMode(!isDarkMode)} className="p-2.5 rounded-xl bg-slate-50 dark:bg-slate-800">{isDarkMode ? <Sun className="w-5 h-5 text-yellow-500" /> : <Moon className="w-5 h-5" />}</button>
      ...
      <main className="max-w-4xl mx-auto px-6 py-16">
        ...
        <Card className="p-8 bg-white/90 dark:bg-slate-900/90 border-slate-200 dark:border-slate-800 mb-12 shadow-xl rounded-3xl">
             <h3 className="text-xl font-bold mb-6 flex items-center gap-2"><FolderPlus className="text-blue-600" /> Start a Group</h3>
             <div className="flex gap-3">
               <Input placeholder="Goa Trip" value={newGroupName} onChange={e => setNewGroupName(e.target.value)} className="h-12 rounded-xl dark:bg-slate-800" />
               <Button onClick={async () => { await fetch("/api/groups", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ name: newGroupName }) }); setNewGroupName(""); fetchGroups(); }} className="h-12 px-8 bg-blue-600 rounded-xl font-bold"><Plus className="mr-2 h-5 w-5" />Create</Button>
             </div>
        </Card>
        <div className="grid md:grid-cols-2 gap-6">
          {groups.map(g => (
            <Card key={g.id} className="p-6 bg-white/90 dark:bg-slate-900/90 border-slate-200 dark:border-slate-800 hover:border-blue-500 cursor-pointer transition-all rounded-3xl shadow-md group" onClick={() => setSelectedGroupId(g.id)}>
              <div className="flex justify-between items-center mb-4"><h3 className="font-black text-xl text-slate-800 dark:text-white">{g.name}</h3><ChevronRight className="w-6 h-6 text-slate-300 group-hover:translate-x-1 transition-all" /></div>
              <p className="text-sm font-bold text-slate-500"><Users className="inline w-4 h-4 mr-1" />{g.member_count} Members</p>
            </Card>
          ))}
        </div>
      </main>
  ...
  return (
    <MainWrapper isDarkMode={isDarkMode}>
      <header className="bg-white/80 dark:bg-slate-900/80 border-b dark:border-slate-800 sticky top-0 py-4 z-10">
        <div className="max-w-7xl mx-auto px-6 flex items-center justify-between">
          <div className="flex items-center gap-4 cursor-pointer" onClick={() => setSelectedGroupId(null)}>
            ...
            {editingGroupName ? (
              <div className="flex items-center gap-2">
                <Input value={editedName} onChange={(e) => setEditedName(e.target.value)} onKeyPress={(e) => e.key === "Enter" && updateGroupName()} className="h-10 w-48 font-bold dark:bg-slate-800" autoFocus />
          ...
          <div className="flex items-center gap-3">
             <button onClick={() => setIsDarkMode(!isDarkMode)} className="p-2.5 rounded-xl bg-slate-50 dark:bg-slate-800 transition-all">{isDarkMode ? <Sun className="w-5 h-5 text-yellow-500" /> : <Moon className="w-5 h-5" />}</button>
             {selectedGroup?.canEdit && <button onClick={() => { navigator.clipboard.writeText(`${window.location.origin}/?groupId=${selectedGroup!.id}`); alert("Invite link copied!"); }} className="p-2.5 text-slate-400 hover:text-blue-600 rounded-xl transition-colors"><Share2 className="w-5 h-5" /></button>}
      ...
      <main className="max-w-6xl mx-auto px-6 py-10">
        {selectedGroup && !selectedGroup.canEdit && <Card className="p-5 bg-blue-50/50 dark:bg-blue-900/20 mb-8 flex items-center gap-4 rounded-2xl shadow-sm border border-blue-100 dark:border-blue-900/50"><Lock className="w-6 h-6 text-blue-600" /><p className="font-bold">Read-Only View</p></Card>}
        
        <div className="grid lg:grid-cols-12 gap-8">
          <div className="lg:col-span-4 space-y-6">
            <Card className="p-8 bg-white/90 dark:bg-slate-900/90 border dark:border-slate-800 shadow-xl rounded-3xl">
              ...
              {selectedGroup?.canEdit && (
                <div className="flex gap-2 mb-8">
                  <Input placeholder="Add name" value={newMemberName} onChange={e => setNewMemberName(e.target.value)} onKeyPress={(e) => e.key === "Enter" && fetch(`/api/groups/${selectedGroup!.id}/members`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ name: newMemberName }) }).then(() => {setNewMemberName(""); fetchGroupDetail(selectedGroup!.id);})} className="h-11 rounded-xl dark:bg-slate-800" />
                  <Button size="icon" onClick={() => fetch(`/api/groups/${selectedGroup!.id}/members`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ name: newMemberName }) }).then(() => {setNewMemberName(""); fetchGroupDetail(selectedGroup!.id);})} className="h-11 w-11 bg-blue-600 rounded-xl"><Plus className="w-5 h-5" /></Button>
                </div>
              )}
              <div className="space-y-3">
                {selectedGroup?.members.map(m => (
                  <div key={m.id} className="p-4 bg-slate-50 dark:bg-slate-800/50 border border-slate-100 dark:border-slate-700 rounded-2xl flex justify-between group items-center transition-colors">
          ...
          <div className="lg:col-span-8 space-y-8">
            <div className="grid grid-cols-3 gap-6">
              <Card className="p-6 bg-white/90 dark:bg-slate-900/90 text-center shadow-lg rounded-3xl border dark:border-slate-800"><p className="text-xs text-slate-400 font-black mb-1 uppercase">Spent</p><p className="text-2xl font-black text-slate-800 dark:text-white">{formatINR(totalExpenses)}</p></Card>
              <Card className="p-6 bg-white/90 dark:bg-slate-900/90 text-center shadow-lg rounded-3xl border dark:border-slate-800"><p className="text-xs text-slate-400 font-black mb-1 uppercase">Debts</p><p className="text-2xl font-black text-orange-500">{settlements.length}</p></Card>
              <Card className="p-6 bg-white/90 dark:bg-slate-900/90 text-center shadow-lg rounded-3xl border dark:border-slate-800"><p className="text-xs text-slate-400 font-black mb-1 uppercase">People</p><p className="text-2xl font-black text-blue-600">{selectedGroup?.members.length}</p></Card>
            </div>

            {selectedGroup?.canEdit && (
              <Card className="p-8 bg-white/90 dark:bg-slate-900/90 border dark:border-slate-800 shadow-xl rounded-3xl border-t-4 border-t-blue-600">
                ...
                <div className="space-y-6">
                  <div className="grid md:grid-cols-2 gap-6">
                    <Input placeholder="Lunch, Fuel..." value={newExpense.description} onChange={e => setNewExpense({...newExpense, description: e.target.value})} className="h-12 rounded-xl dark:bg-slate-800" />
                    <Input type="number" placeholder="₹ Total bill" value={newExpense.amount} onChange={e => setNewExpense({...newExpense, amount: e.target.value})} className="h-12 rounded-xl dark:bg-slate-800" />
                  </div>
                  <Select value={newExpense.paidBy} onValueChange={v => setNewExpense({...newExpense, paidBy: v})}>
                    <SelectTrigger className="h-12 rounded-xl dark:bg-slate-800"><SelectValue placeholder="Who paid?" /></SelectTrigger>
                    <SelectContent>{selectedGroup.members.map(m => (<SelectItem key={m.id} value={m.id.toString()}>{m.name}</SelectItem>))}</SelectContent>
                  </Select>

                  <div className="flex gap-4">
                    <Button onClick={() => setNewExpense({...newExpense, splitType: 'equal'})} variant={newExpense.splitType === 'equal' ? 'default' : 'outline'} className="flex-1 rounded-xl font-bold transition-all">Equal Split</Button>
                    <Button onClick={() => setNewExpense({...newExpense, splitType: 'custom'})} variant={newExpense.splitType === 'custom' ? 'default' : 'outline'} className="flex-1 rounded-xl font-bold transition-all">Custom Split (₹)</Button>
                  </div>

                  <div className="space-y-4">
                    <Label className="text-xs font-black uppercase opacity-50 ml-1">Split Among Members:</Label>
                    <div className="grid md:grid-cols-2 gap-3">
                      {selectedGroup.members.map(m => {
                        const isIncluded = newExpense.splitAmong.includes(m.id);
                        return (
                          <div key={m.id} className={`p-4 border rounded-2xl dark:border-slate-800 transition-all ${isIncluded ? 'border-blue-400 bg-blue-50/10 dark:bg-blue-900/10' : 'bg-slate-50/50 dark:bg-slate-800/50'} flex flex-col gap-2`}>
                            <button type="button" onClick={() => toggleSplitMember(m.id)} className="flex items-center gap-2 font-black text-sm">
                              {isIncluded ? <CheckCircle2 className="text-blue-600 w-5 h-5" /> : <div className="w-5 h-5 border-2 rounded-full" />}
                              {m.name}
                            </button>
                            {newExpense.splitType === 'custom' && isIncluded && (
                              <Input 
                                  type="number" placeholder="₹ 0" className="h-10 rounded-lg text-xs dark:bg-slate-900" 
                  ...
                  <Button onClick={addExpense} className="bg-blue-600 w-full h-14 text-lg font-black rounded-2xl shadow-xl hover:bg-blue-700 transition-all">Record Record</Button>
                </div>
              </Card>
            )}

            <div className="flex gap-8 border-b-2 dark:border-slate-800">
              <button onClick={() => setActiveTab("expenses")} className={`pb-4 text-sm font-black transition-all relative ${activeTab === "expenses" ? "text-blue-600" : "text-slate-400"}`}>EXPENSES {activeTab === "expenses" && <div className="absolute bottom-0 w-full h-1 bg-blue-600 rounded-full" />}</button>
              <button onClick={() => setActiveTab("settlements")} className={`pb-4 text-sm font-black transition-all relative ${activeTab === "settlements" ? "text-blue-600" : "text-slate-400"}`}>SETTLEMENTS {activeTab === "settlements" && <div className="absolute bottom-0 w-full h-1 bg-blue-600 rounded-full" />}</button>
              <button onClick={() => setActiveTab("history")} className={`pb-4 text-sm font-black transition-all relative ${activeTab === "history" ? "text-blue-600" : "text-slate-400"}`}>HISTORY {activeTab === "history" && <div className="absolute bottom-0 w-full h-1 bg-blue-600 rounded-full" />}</button>
            </div>

            {activeTab === "expenses" && (
              <div className="space-y-4">
                <div className="relative"><Search className="absolute left-4 top-4 text-slate-400 w-6 h-6" /><Input placeholder="Search..." className="pl-12 h-14 rounded-2xl mb-6 dark:bg-slate-900" value={searchTerm} onChange={e => setSearchTerm(e.target.value)} /></div>
                {filteredExpenses.map(e => (
                  <Card key={e.id} className="p-6 bg-white/90 dark:bg-slate-900/90 border dark:border-slate-800 rounded-3xl shadow-md flex justify-between items-center group transition-all">
                    <div className="flex items-center gap-4"><div className="w-12 h-12 rounded-2xl bg-blue-50 dark:bg-blue-900/20 flex items-center justify-center text-blue-600 font-bold shadow-inner">₹</div><div><h3 className="font-black text-lg text-slate-800 dark:text-white">{e.description}</h3><p className="text-xs font-bold text-slate-400">Paid by <span className="text-blue-500">{getMemberName(e.paid_by_member_id)}</span></p></div></div>
                    <div className="flex items-center gap-6 text-right"><div><p className="font-black text-2xl text-slate-800 dark:text-white">{formatINR(e.amount)}</p><p className="text-xs text-slate-400 uppercase font-black">{e.split_type === 'custom' ? 'Custom bill' : `Split by ${e.splitAmong.length}`}</p></div>{selectedGroup?.canEdit && <button onClick={async () => { await fetch(`/api/expenses/${e.id}`, { method: "DELETE" }); fetchGroupDetail(selectedGroup!.id); }} className="text-slate-200 hover:text-red-500 opacity-0 group-hover:opacity-100 transition-all"><Trash2 className="w-5 h-5" /></button>}</div>
                  </Card>
                ))}
              </div>
            )}
            ...
            {activeTab === "settlements" && (
              <div className="space-y-4">
                {settlements.map((s, idx) => {
                  ...
                  return (
                    <Card key={idx} className="p-6 bg-white/90 dark:bg-slate-900/90 border dark:border-slate-800 shadow-lg rounded-3xl flex justify-between items-center transition-all hover:border-blue-400">
                ...
                {settlements.length === 0 && <Card className="p-20 text-center text-blue-600 font-black border-2 border-dashed border-blue-100 dark:border-blue-900 rounded-3xl text-xl bg-blue-50/10">ALL DEBTS SETTLED! 🎉</Card>}
              </div>
            )}
            ...
            {activeTab === "history" && selectedGroup && (
               <div className="space-y-4">
                 {selectedGroup.payments.map(p => (
                   <Card key={p.id} className="p-6 bg-white/90 dark:bg-slate-900/90 border dark:border-slate-800 flex justify-between items-center rounded-3xl shadow-sm group transition-all">
